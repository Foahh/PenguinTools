...
internal static class OptionCommands
{
    ...
    private sealed class OptionConvertCliOptions
    {
        ...
        {
            Description = "When true (default), read options.json from the input directory. When false, skip the file and use defaults plus CLI flags; --option-name is then required.",
            DefaultValueFactory = _ => true
        };

        internal Option<string?> OptionName { get; } = new("--option-name")
        {
            Description = "Four-letter bundle name. Required when --load-json is false; otherwise overrides options.json optionName."
        };
        ...
        {
            if (parseResult.GetValue(OptionName) is { } optionName && optionName.Length > 0)
            {
                if (optionName.Length != 4)
                {
                    return "--option-name must be exactly four characters.";
                }
            ...
            if (parseResult.GetValue(BatchSize) is int batchSize)
            {
                document.BatchSize = batchSize;
            }

            if (parseResult.GetValue(ConvertChart) is bool convertChart)
            {
                document.ConvertChart = convertChart;
            }

            if (parseResult.GetValue(ConvertAudio) is bool convertAudio)
            {
                document.ConvertAudio = convertAudio;
            }

            if (parseResult.GetValue(ConvertJacket) is bool convertJacket)
            {
            ...
            if (parseResult.GetValue(UltimaEventId) is int ultimaEventId)
            {
                document.UltimaEventId = ultimaEventId;
            }

            if (parseResult.GetValue(WeEventId) is int weEventId)
            {
                document.WeEventId = weEventId;
            }
    ...
    {
        var command = new Command("option", "Export an option bundle from scanned charts, using options.json by default.");
    ...
    private static Command BuildOptionConvertCommand()
    {
        ...
        var command = new Command("convert", "Scan charts and export the option bundle, using options.json by default or CLI configuration when --load-json is false.");
        ...
        command.SetAction(async (parseResult, cancellationToken) =>
        {
            ...
            return await CliOperations.ExecuteAsync("option convert", outputFormat, async (runtime, ct) =>
            {
                ...
                if (loadJson)
                {
                    ...
                    var fromFile = await JsonSerializer.DeserializeAsync<OptionDocument>(optionsStream, CliJsonSerializerContext.Default.OptionDocument, ct);
                    if (fromFile is null)
                    {
                        return new CliCommandOutcome(
                ...
                else
                {
                    ...
                    {
                        return new CliCommandOutcome(
                            OperationResult.Failure().WithDiagnostics(CliDiagnostics.SnapshotFromMessage("With --load-json false, --option-name (exactly four characters) is required.")),
                ...
                {
                    return new CliCommandOutcome(
                        OperationResult.Failure().WithDiagnostics(CliDiagnostics.SnapshotFromMessage("No export actions are enabled after applying options.json and command-line overrides.")),
